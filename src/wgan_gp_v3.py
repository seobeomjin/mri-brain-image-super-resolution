...
from data_prep.patchloader import patching, depatching
...
#         real = torch.tensor(torch.rand(logit_real.size())*0.25 + 0.85)
    ...
    def training(self, dataloaders,
        ...
        if pretrainedG != ' ':
            ...
            step = int(re.sub("my_models/pretrained_G_step|my_models/WGAN_G_step", "", pretrainedG))  #start from the pretrained model's step
            train_loss=[]
            train_D_loss=[]
            val_loss=[]
            val_D_loss=[]
                    ...
                    for lr_data, hr_data in dataloaders[phase][fold]:
                        ...
                        for lr_patches, hr_patches in patch_loader:
                            ...
                            if phase == 'train':
                                ...
                                ##########추가 
                                    f=open('example_images/patched/pat_image_lr_step{}.txt'.format(step),'wb')
                                    pickle.dump(lr_patches.cpu().numpy() ,f)
                                    f.close()
                                    f=open('example_images/patched/pat_image_sr_step{}.txt'.format(step),'wb')
                                    pickle.dump(sr_patches.cpu().numpy() ,f)
                                    f.close()
                                    f=open('example_images/patched/pat_image_hr_step{}.txt'.format(step),'wb')
                                    pickle.dump(hr_patches.cpu().numpy() ,f)
                               
                               ###########추가 
                                if (step % 1000) == 0:
                                    if self.ngpu > 1:
                                        torch.save(self.netG.module.state_dict(),'my_models/WGAN_G_step{}'.format(step))
                                        torch.save(self.netD.module.state_dict(),'my_models/WGAN_D_step{}'.format(step))
                                    else:
                                        torch.save(self.netG.state_dict(),'my_models/WGAN_G_step{}'.format(step))
                                        torch.save(self.netD.state_dict(),'my_models/WGAN_D_step{}'.format(step)) 
                                ...
                                if (step == max_step):
                                    ...
                                    if self.ngpu > 1:
                                        torch.save(self.netG.module.state_dict(),'my_models/final_model_G')
                                        torch.save(self.netD.module.state_dict(),'my_models/final_model_D')
                                    else:
                                        torch.save(self.netG.state_dict(),'my_models/final_model_G')
                                        torch.save(self.netD.state_dict(),'my_models/final_model_D')
