...
Original file is located at
    https://colab.research.google.com/drive/1ZscDfdXNizeM4zjcM3R3f_dgnQCkmAhf
"""
...
def training_pre(model, dataloaders, dataset_sizes,
    ...
    if pretrained != ' ':
        ...
        if (device.type == 'cuda') and (ngpu > 1):   ###############수정> : 1에 등호 넣음 
            model = nn.DataParallel(model, list(range(ngpu)))
        step = int(re.sub('my_models/pretrained_G_step|my_models/pretrained_D_step', "", pretrained))  #start from the pretrained model's step  ###########수정 
        print("step : ", step)
    else:
        step = 0
           
    #step = 249999    ######## just experiment
    while(step < max_step):
...
#                                 print('Step: {}, loss= {:.4f}'.format(step, loss.item()))
                            if (step % int(max_step // 10)) ==0:
                                ...
                                torch.save(model,'my_models/pretrained_G_step{}'.format(step))
                            ...
                            if (step == max_step):
                                ...
                                if ngpu > 1:
                                    torch.save(model.module.state_dict(),'my_models/pretrained_G_step{}'.format(step))
                                else:
                                    torch.save(model.state_dict(),'my_models/pretrained_G_step{}'.format(step))
                                return model
                    # statistics
